...
|---|---|---|
| id | UUID PK | |
| name | VARCHAR(20) | Свежие овощи, Фрукты, Бакалея, Сухофрукты, Баранина (халяль), Говядина (халяль), Курица (халяль), Индейка (халяль), Заморозка |
| image_url | VARCHAR(200) | nullable |
| position | INT | порядок отображения |
| created_at / updated_at | TIMESTAMP | |

> **Текущий seed (grocery delivery):** 9 категорий выше, по 10 товаров в каждой. Акцент на органик/эко/халяль вынесен в `products.description` и теги (`Органик`, `Эко`, `Халяль`, `Веган`, `Премиум`, `Новинка`).
...
|---|---|---|
| id | UUID PK | |
| label | VARCHAR(20) UNIQUE | "Органик", "Эко", "Халяль", "Веган", "Премиум", "Новинка" |
| color | VARCHAR(20) NOT NULL UNIQUE | hex-код, например `#4c8c46` |
| position | INT | порядок отображения |

> **Примечание:** `color` персистится в БД (миграция `2026-06-17-001-fix-tags-add-color` в backend-репо — до неё колонки не было и `/tags` падал с 500). `emoji` остаётся UI-only полем на фронтенде, в БД не хранится.
