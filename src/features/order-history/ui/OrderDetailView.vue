...
import { STATUS_META, PAYMENT_LABEL } from '@/entities/order'
        ...
        <template v-for="(step, i) in STEPS" :key="step.key">
          <div class="flex flex-col items-center gap-1 flex-1 min-w-0">
            <div
              ...
              :class="STATUS_META[order.status].step >= (i + 1)
                ? 'bg-accent border-accent text-white'
                : 'bg-canvas border-line text-faint'"
            >
              <span v-if="STATUS_META[order.status].step > (i + 1)">✓</span>
              <span v-else>{{ i + 1 }}</span>
            </div>
            <span class="text-[10px] text-center text-faint leading-tight hidden sm:block">{{ step.label }}</span>
          </div>
          <div
            v-if="i < STEPS.length - 1"
            class="h-0.5 flex-1 mt-[-12px] transition-colors"
            :class="STATUS_META[order.status].step > (i + 1) ? 'bg-accent' : 'bg-line'"
          />
        </template>
      </div>
    </div>
